...
def build(source, dist, html):
    ...
    for album in source.listdir(filter=unipath.DIRS):
        ...
        html.write('<header>{}</header>'.format(album.name))
        ...
        for photo in album.listdir(filter=unipath.FILES):
            ...
            html.write('<figure>')
            html.write('<a href="{}">'.format(path_big))
            html.write('<picture>')
            html.write('<source src="{}" media=(min-width:992px)>'.format(path_ico_big))
            html.write('<source src="{}" media=(max-width:991px)>'.format(path_ico_small))
            html.write('<img src="{}" alt="{}" title="{}">'.format(path_ico_big, photo.name, photo.stem))
            html.write('</picture>')
            html.write('</a>')
            html.write('<figcaption>')
            html.write('<a href="{}">HD: {}</a>'.format(path_org, photo.name))
            html.write('</figcaption>')
            html.write('</figure>')
        html.write('</section>')
